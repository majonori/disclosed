...
inline bool reduce(){
  int pos = 1;
  while(gamma[pos] == 0 && pos <= cnt) gamma[pos] = alpha[pos] - bbeta[pos], ids += prod[pos - 1] * gamma[pos], ++pos;
...
//	print();
  for(int i = 2; i <= cnt; ++i){
    ...
    int idp = prod[i - 1];
    for(int j = 1; j <= cnt; ++j) gamma[j] = alpha[j] - (i == j), bbeta[j] = (i == j);
    ids = sigma0 - 1 - idp;
...
//		print();
    if(N % (prime[i] - 1) == 0){
      getpos(prime[i] - 1);
      for(int j = 1; j <= cnt; ++j) gamma[j] = alpha[j] - bbeta[j];
      ids = sigma0 - 1 - id;
...
//		print();
  ...
  while(pushup()) if(N % (now + 1) != 0 && Miller_Rabin(now + 1)){
    for(int i = 1; i <= cnt; ++i) gamma[i] = alpha[i] - bbeta[i];
    ids = sigma0 - 1 - id;
