...
加上 EI 的多元多项式优化可以做到 $O(s\sigma_0(n)\log\sigma_0(n)+\sigma_0(n)\log n)$。

不过这篇题解还是讲的多元 exp 优化做法，简单的做法再后面有补充。
...
当然，这题不用你真的让你写这么复杂的优化，那 $\Theta(\sigma_0^2(n))$ 有太慢了怎么办？

我们仍可用多元的方法思考，  
考虑 $\sigma_0(n)$ 次暴力卷积，不过其实每次只会更新指数比它高的项。  
所以跑不到 $\Theta(\sigma_0^2(n))$ 。  

时间复杂度实际上相当于是因子的因子个数，即为 $d_3(n)$。

还有一种比较简单的方法实现 $d_3(n)$ 的时间复杂度。  
就是暴力递推计算 exp 即可。
...
以下两种做法都是 $O(d_3(n)+\sigma_0(n)\log n)$ 的时间复杂度，  
不过有个 $s$ 的常数，你可以通过状压等方法去掉。
